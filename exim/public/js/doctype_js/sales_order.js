frappe.ui.form.on("Sales Order", {
    setup(frm) {
        ...
        frm.fields_dict.items.grid.get_field("ref_no").get_query = function (doc, cdt, cdn) {
        ...
        if (frm.doc.customer_address || frm.doc.shipping_address_name) {
            ...
            frappe.db.get_value(
                ...
                frappe.db.get_value(
                    ...
                    if (
                        r.message.country === "India" ||
                        d.message.country === "India"
                    ) {
    ...
    onload_post_render(frm) {
        ...
        let $group = frm.page.get_inner_group_button("Make");

        if ($group) {
            let items = $group.find(".dropdown-menu").children("li");

            items.each(function () {
                let anchor = $(this).find("a");

                if (anchor.html() === "Delivery") {
                    $(this).remove();
                }
            });
        }
    ...
    box_cal(frm) {
        frm.doc.items.forEach(function (d, i) {
            ...
            if (i === 0) {
                ...
                d.packages_from =
                    Math.round(frm.doc.items[i - 1].packages_to + 1);

                d.packages_to =
                    Math.round(d.packages_from + d.no_of_packages - 1);
            }
        });
        ...
        frm.doc.items.forEach(function (d, i) {
            ...
            if (d.palleted) {
                ...
                if (i === 0) {
                    ...
                    d.pallet_no_from =
                        Math.round(frm.doc.items[i - 1].pallet_no_to + 1);

                    d.pallet_no_to =
                        Math.round(d.pallet_no_from + d.total_pallets - 1);
                }
            }
        });
        ...
        frappe.run_serially([
            () => {
                ...
                frappe.model.set_value(
                    ...
                    cdn,
                    "total_pallets",
                    Math.round(d.qty / d.pallet_size)
                );
            },
    ...
    qty(frm, cdt, cdn) {
        ...
        if (d.qty > 0 && d.packing_size > 0) {
            ...
            frappe.model.set_value(
                ...
                cdn,
                "no_of_packages",
                flt(d.qty / d.packing_size)
            );
        }
    },

    packing_size(frm, cdt, cdn) {
