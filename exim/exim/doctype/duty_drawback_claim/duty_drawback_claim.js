...
frappe.ui.form.on("Duty DrawBack Claim", {
	get_rodtep_entries(frm) {
		if (frm.doc.rodtep_details) {
			for (let j = frm.doc.rodtep_details.length - 1; j >= 0; j--) {
				frm.get_field("rodtep_details").grid.grid_rows[j].remove();
			}
		}

		frappe.call({
			method:
				"exim.exim.doctype.duty_drawback_claim.duty_drawback_claim.journal_entry_list",
			args: {
			...
			callback: function (r) {
				if (r.message) {
					r.message.forEach(function (res) {
						let childTable = frm.add_child("rodtep_details");
